# =============================================================================
# 01_synthetic.jl — Synthetic changepoint detection
#
# Replicates El-Yaagoubi et al. (2025) exactly, plus novel phase-space pipeline.
# All 5 metrics computed in ONE pass per replicate. Parallelised over replicates.
# Checkpoints saved after calibration and MC evaluation per scenario.
#
# Run: julia --threads=auto --project=. 01_synthetic.jl
# =============================================================================
...
gen_s1(; R=R_TRIALS, T=T_TRIAL, rstar=R_STAR, seed=1) =
    [ar2(T, r <= rstar ? 10.0 : 40.0;
         rng=MersenneTwister(seed*1000+r)) for r in 1:R]
...
function gen_s3(; R=R_TRIALS, T=T_TRIAL, rstar=R_STAR, seed=1)
    [r <= rstar ?
        ar2(T, 15.0; rng=MersenneTwister(seed*1000+r)) :
...
# =============================================================================
# 2. Core pipeline — all 5 metrics in ONE pass
# =============================================================================
...
function all_metrics(trials::Vector{Vector{Float64}}; bw=5, r0=R0)
    ...
    D_L1  = zeros(R-1); D_L2 = zeros(R-1)
    D_W   = zeros(R-1); D_T  = zeros(R-1)
    S_sum = copy(Ps[1])
    for r in 1:R-1
        P_avg  = S_sum ./ r
        P_next = Ps[r+1]
        diff   = P_avg .- P_next
        D_L1[r] = sum(abs, diff)
        D_L2[r] = sqrt(sum(abs2, diff))
        D_W[r]  = sum(abs, sort(P_avg) .- sort(P_next))
        d1 = sublevel_ph(P_avg).H0
        d2 = sublevel_ph(P_next).H0
    ...
    return (L1=D_L1, L2=D_L2, W=D_W, T=D_T, B=D_B)
end
...
# =============================================================================
# 4. MC evaluation with progress counter and checkpoints
# =============================================================================

const METRICS = (:L1, :L2, :W, :T, :B)

function mc_eval_all(gen, gnull, scenario_key;
                     B=500, B0=200, alpha=0.01, r0=R0, rstar=R_STAR)

    # --- Threshold calibration ---
    τ = cached("syn_$(scenario_key)_tau") do
    ...
    # --- MC evaluation ---
    results = cached("syn_$(scenario_key)_results") do
...
SCENARIOS = [
    ("s1", "1: frequency shift (10→40 Hz)",  gen_s1, gen_null_s1,
     "Spectral peak moves; topology unchanged"),
    ("s2", "2: quasiperiodic onset",          gen_s2, gen_null_s2,
     "New spectral peak; new H₁ generator"),
    ("s3", "3: orbit collapse (AR2→AR1)",     gen_s3, gen_null_s3,
     "Peak disappears; loop collapses"),
]

println("Threads: $(Threads.nthreads())")
println("=== Synthetic experiments  R=$R_TRIALS × T=$T_TRIAL, r*=$R_STAR ===\n")
...
for (key, name, gen, gnull, desc) in SCENARIOS
    ...
    RES[name] = res
    for (sym, label) in [(:L1,"L1"), (:L2,"L2"), (:W,"W"),
                          (:T,"T (topo H₀)"), (:B,"T-PS (phase H₁)")]
        r = res[sym]
        @printf("  %-18s  med Δ=%6.1f  IQR=%6.1f  miss=%5.1f%%\n",
...
COLORS = Dict(:L1=>:steelblue, :L2=>:royalblue,
              :W=>:mediumpurple, :T=>:firebrick, :B=>:darkorange)
LABELS = Dict(:L1=>"L1", :L2=>"L2", :W=>"W (spectral)",
              :T=>"T (topo H₀)", :B=>"T-PS (phase-space H₁)")
...
for (row, (_, name, gen, _, desc)) in enumerate(SCENARIOS)
    ...
    ax = Axis(fig[row, 1];
              ...
              titlesize = 11)
    for sym in (:L1, :L2, :W, :T, :B)
        Dn = normalise(getfield(scores, sym), R0)
        lines!(ax, Dn; color=COLORS[sym],
               linewidth=(sym==:B ? 2 : 1), label=LABELS[sym])
    end
    vlines!(ax, [R_STAR]; color=:black, linestyle=:dash, label="True r*")
    row == 1 && axislegend(ax; position=:rt, framevisible=false, labelsize=9)
end

save("plots/empirical/synthetic_changepoint.pdf", fig)
println("Saved → plots/empirical/synthetic_changepoint.pdf")
...
println("\n=== LaTeX table ===")
@printf("%-32s & L1 & L2 & W & T & T-PS \\\\\n", "Scenario")
println("\\midrule")
for (_, name, _, _, _) in SCENARIOS
    res = RES[name]
    @printf("%-32s & %.1f & %.1f & %.1f & %.1f & %.1f \\\\\n",
            name,
            res[:L1].miss*100, res[:L2].miss*100,
            res[:W].miss*100,  res[:T].miss*100,
            res[:B].miss*100)
end
